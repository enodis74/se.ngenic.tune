...
module.exports = class MyTrackOptoDriver extends Homey.Driver {

  /**
   * onInit is called when the driver is initialized.
   */
  async onInit() {
    this.log('Track Opto driver has been initialized');
  }
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        for (let j = 0; j < tune.gateway.children.length; j++) {
          if (tune.gateway.children[j].type === NgenicTunesClient.NODE_TYPE_SENSOR &&
              tune.gateway.children[j].device.type === NgenicTunesClient.DEVICE_TYPE_TRACK_OPTO) {
                res.push({
                  name: tune.name,
                  data: {
